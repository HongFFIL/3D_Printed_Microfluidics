...
# Apply augmentations to a cell image
def augment_image(cell):
    augmentations = []

    # Vertical Flip
    augmentations.append(cv2.flip(cell, 0))

    # Horizontal Flip
    augmentations.append(cv2.flip(cell, 1))

    # Random Rotation
    angle = random.choice([90, 180, 270])
    h, w = cell.shape[:2]
    rotation_matrix = cv2.getRotationMatrix2D((w / 2, h / 2), angle, 1)
    rotated_cell = cv2.warpAffine(cell, rotation_matrix, (w, h))
    augmentations.append(rotated_cell)
...
def create_circular_blending_mask(cell):
    ...
    # mask = cv2.GaussianBlur(mask, (3, 3), 1)  # Smooth edges
    return mask

# Extract yeast cells and apply augmentations
def extract_yeast_cells(image, bounding_boxes):
    ...
    for box in bounding_boxes:
        ...
        x1 = max(0, int((center_x - box_width / 2) * w) - 30)
        x2 = min(w, int((center_x + box_width / 2) * w) + 30)
        y1 = max(0, int((center_y - box_height / 2) * h) - 30)
        y2 = min(h, int((center_y + box_height / 2) * h) + 30)

        if center_x < 100 / w or center_x > (w - 100) / w:
            continue
        if not (330 <= int(center_y * h) <= 800):
            continue
        ...
        # Extract the original cell
        original_cell = image[y1:y2, x1:x2]
        augmented_cells = augment_image(original_cell)

        for aug_cell in augmented_cells:
            yeast_cells.append((label, aug_cell, box_width, box_height, original_box, center_y))
    ...
    for cell_data in selected_cells:
        ...
        for _ in range(100):
            ...
            if not any((x_offset < eb[2] and x_offset + cell_w > eb[0] and y_offset < eb[3] and y_offset + cell_h > eb[1]) for eb in existing_boxes):
                ...
                # Rotate the cell image
                rotation_matrix = cv2.getRotationMatrix2D(center, angle, 1.0)
                rotated_cell = cv2.warpAffine(cell_part, rotation_matrix, (cell_w, cell_h), flags=cv2.INTER_LINEAR)

                blended_roi = cv2.add(background_part, rotated_cell)
...
def create_blended_images(background_path, class_folders, output_image_folder, output_label_folder, max_images):
    ...
    while image_count < max_images:
        ...
        output_image_path = os.path.join(output_image_folder, f"ba_med_{image_count + 1}.jpg")
        output_label_path = os.path.join(output_label_folder, f"ba_med_{image_count + 1}.txt")
...
output_image_folder = '/home/hong_data1/Documents/Delaney/yeast_viability/liveoven_augment/med/images/'  # Replace with actual path
output_label_folder = '/home/hong_data1/Documents/Delaney/yeast_viability/liveoven_augment/med/labels/'  # Replace with actual path
max_images = 700
